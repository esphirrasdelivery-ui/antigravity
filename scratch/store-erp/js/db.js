...
const DB = {
  ...
  // ===== PRODUTOS =====
  getProdutos() {
    return _getLocal('loja_produtos');
  },
  saveProdutos(data) {
    _saveLocal('loja_produtos', data);
  },
  addProduto(produto) {
    ...
    _saveFirebase('loja_produtos', produto.id, produto);
    return produto;
  },
  updateProduto(id, updates) {
    ...
    if (idx >= 0) {
      ...
      this.saveProdutos(produtos);
      _saveFirebase('loja_produtos', id, produtos[idx]);
      return produtos[idx];
    }
    return null;
  },
  deleteProduto(id) {
    ...
    this.saveProdutos(produtos);
    _deleteFirebase('loja_produtos', id);
  },
  ...
  // ===== FICHAS TÉCNICAS =====
  getFichas() {
    return _getLocal('loja_fichas');
  },
  saveFichas(data) {
    _saveLocal('loja_fichas', data);
  },
  addFicha(ficha) {
    ...
    _saveFirebase('loja_fichas', ficha.id, ficha);
    return ficha;
  },
  updateFicha(id, updates) {
    ...
    if (idx >= 0) {
      ...
      this.saveFichas(fichas);
      _saveFirebase('loja_fichas', id, fichas[idx]);
      return fichas[idx];
    }
    return null;
  },
  deleteFicha(id) {
    ...
    this.saveFichas(fichas);
    _deleteFirebase('loja_fichas', id);
  },

  // ===== MOVIMENTAÇÕES DE ESTOQUE =====
  getMovimentacoes() {
    return _getLocal('loja_movimentacoes');
  },
  saveMovimentacoes(data) {
    _saveLocal('loja_movimentacoes', data);
  },
  addMovimentacao(mov) {
    ...
    _saveFirebase('loja_movimentacoes', mov.id, mov);
  ...
  // ===== PRODUÇÕES =====
  getProducoes() {
    return _getLocal('loja_producoes');
  },

  // ===== PEDIDOS DE LOJA =====
  getPedidos() {
    return _getLocal('loja_pedidos');
  },
  savePedidos(data) {
    _saveLocal('loja_pedidos', data);
  },
  updatePedido(id, updates) {
    ...
    if (idx >= 0) {
      ...
      _saveFirebase('loja_pedidos', id, pedidos[idx]);
      return pedidos[idx];
    }
    return null;
  },
  ...
  getMetas() {
    return _getLocal('loja_lojas_metas') || [];
  },
  saveMetas(data) {
    _saveLocal('loja_lojas_metas', data);
  },
  updateMeta(loja, configObj) {
    ...
    _saveFirebase('loja_lojas_metas', metaRec.id, metaRec);
  },

  // ===== LOTES DE ARMAZENAMENTO =====
  getLotes() {
    return _getLocal('loja_lotes');
  },
  saveProducoes(data) {
    _saveLocal('loja_producoes', data);
  },
  addProducao(producao) {
    ...
    _saveFirebase('loja_producoes', producao.id, producao);
    return producao;
  },
  _gerarLote() {
    const d = new Date();
  ...
  addLote(lote) {
    ...
    _saveFirebase('loja_lotes', lote.id, lote);
    return lote;
  },
  updateLote(id, updates) {
    ...
    if (idx >= 0) {
      ...
      this.saveLotes(lotes);
      _saveFirebase('loja_lotes', id, lotes[idx]);
      return lotes[idx];
    }
    return null;
  },
  ...
  migrarDadosLocaisParaFirebase() {
    ...
    var colecoes = ['loja_produtos', 'loja_movimentacoes', 'loja_lotes', 'loja_producoes', 'loja_fichas'];
    var total = 0;
